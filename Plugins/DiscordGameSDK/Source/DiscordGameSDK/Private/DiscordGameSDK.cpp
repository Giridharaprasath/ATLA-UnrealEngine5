...
DEFINE_LOG_CATEGORY(LogDiscordGameSDK);

const FName FDiscordGameSDKModule::ModuleName ("DiscordGameSDK");
...
void FDiscordGameSDKModule::StartupModule()
{
	...
	if ensureAlwaysMsgf(!LibraryPath.IsEmpty(), TEXT("Expect LibraryPath to not be empty"))
	{
		...
		{
			UE_LOG(LogDiscordGameSDK, Log, TEXT("Loaded Discord GameSDK DLL [%s]"), *LibraryPath);
		}
		else
		{
			UE_LOG(LogDiscordGameSDK, Error, TEXT("Failed to load Discord GameSDK DLL [%s]"), *LibraryPath);
		}
	}
}
...
FString FDiscordGameSDKModule::GetPathToDLL() const
{
	...
	LibraryPath = FPaths::Combine(*BaseDir, TEXT("Source/ThirdParty/DiscordGameSDKLibrary"));
